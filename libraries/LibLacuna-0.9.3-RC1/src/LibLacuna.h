 ...
 *    - `lsSendLoraSat`, to send a raw LR-FHSS packet
 *    - `lsSendLoraWAN`, to send a LoraWAN packet over Lora
 *    - `lsSendLoraSatWAN`, to send a LoraWAN packet for satellite
...
/* TBD */
int lsStartCW(lsLoraTxParams *params);
int lsSweepCW(lsLoraTxParams *params, uint32_t freq_start, uint32_t freq_stop, uint32_t step, uint16_t duration);
int lsStopCW();

/* Relay */
int lsRelayLora(lsLoraWANParams *loraWanParams, lsLoraTxParams *params, byte *payload, void (*sleep_func)());

/* Receive */
int lsReceiveLora(lsLoraTxParams *params, byte *payload);


/* Sattelite Downlink WakeUp */
int lsSatWakeUp(lsLoraTxParams *params, void (*sleep_func)());
